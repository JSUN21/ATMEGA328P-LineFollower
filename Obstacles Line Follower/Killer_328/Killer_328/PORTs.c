...
void init_ports(void)
{
	...
	*/
	...
	//Entradas sensor IR seguidor de linea
	
	DDRD &= ~(1 << PD2); //sensor extremo der
	DDRD &= ~(1 << PD3);//medio izquierdo
	DDRD &= ~(1 << PD4);//medio derecho
	DDRD &= ~(1 << PD5);// sensor extremo izq
	
	//Entradas sensores IR Obstaculos
	DDRD &= ~(1 << PD6); //Sensor izquierdo obstaculos
	DDRD &= ~(1 << PD7); //Sensor derecho obstaculo
	
	DDRD &= ~(1 << PD0); //Sensor frontal obstaculos
	/*****************************************************************/
